...
Currently, the GRM is split by chromosome; if we set a X-Mbase window we'll have to 
generate a new set of GRMs for each phenotype.
```{bash, eval=FALSE}
#!/bin/bash
for ((i=1;i<=22;i++)); do
...
Now we run GREML. This following script finds the variance for all unrelated individuals, 
then finds the variance for ENSG00000227232.
```{bash, eval=FALSE}
gcta64 --reml --reml-alg 0 --reml-maxit 100 --mpheno 1 --grm grms/unrelated --pheno phenotype --out test
gcta64 --reml --reml-alg 0 --reml-maxit 100 --mpheno 1 --mgrm chrs/test_chrs.txt --pheno phenotype --out test
```
