...
echo $LINE | \
    ...
    gcta64 --reml-bivar $L1 $L2 --thread-num $THREADS --reml-alg 2 --reml-maxit 10000 \
        ...
        # Clean up files
        rm -rf data/${PROJECT}/output/grms/${GENE}
    fi

    # Clean up files
    rm -rf data/${PROJECT}/output/grm_ranges/${GENE}
    rm -rf data/${PROJECT}/output/grm_chrs/${GENE}
    rm -rf data/${PROJECT}/output/mgrms/${GENE}
    rm -rf data/${PROJECT}/output/fin_peds/${GENE}
    )
