...
# Generate Big K GRM
We modify the complete GRM by setting unrelated pairs to 0, < 0.05 as the threshold.  
```{bash, engine.opts='-l', eval=FALSE}
...
hsqs = t(sapply(X=temp, function(file_name) {
                  hsq = read.table(paste('data/', PROJECT, '/output/hsqs/', file_name, sep=''), header=TRUE, nrows=4)
...
```
# Combine the HSQ files and return statistics, slightly different for working with all SNPs
```{r, eval=FALSE}
temp = unlist(list.files(path=paste("data/", PROJECT, "/output/hsqs", sep=""), pattern="*.hsq"))
hsqs = t(sapply(X=temp, function(file_name) {
                  hsq = read.table(paste('data/', PROJECT, '/output/hsqs/', file_name, sep=''), header=TRUE, nrows=3)
                  # Gene ID, SNP, Residual, Phenotype
                  line = c(substring(file_name, 1, nchar(file_name) - 4), 
                           hsq[1,2], hsq[2,2], hsq[3,2])
                  return(line)
            }))
colnames(hsqs) = c("ID", "SNP", "Res", "Phe")
rownames(hsqs) = hsqs[,"ID"]
hsqs = as.data.frame(hsqs)
hsqs = transform(hsqs, SNP=as.numeric(SNP), Res=as.numeric(Res), Phe=as.numeric(Phe))
write.table(hsqs, paste("data/", PROJECT, "/output/results/all_variance.txt", sep=""), quote=FALSE, row.names=FALSE, col.names=TRUE)

avg_snp_h2 = mean(apply(X=hsqs, MARGIN=1, function(row) as.numeric(row[2])/as.numeric(row[4]))) # 0.06808974
```
