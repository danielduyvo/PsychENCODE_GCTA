...
# Combine the HSQ files and return statistics, using a cis and trans window
...
cis_prop = sum(all_hsqs[, "V_Cis"])/sum(all_hsqs[, "V_Cis"] + all_hsqs[, "V_Trans"]) # 0.3395873
trans_prop = sum(all_hsqs[, "V_Trans"])/sum(all_hsqs[, "V_Cis"] + all_hsqs[, "V_Trans"]) # 0.6604127
avg_snp_h2 = mean(all_hsqs[, "SNPh2"]) # 0.1087606
```
...
# Combine the HSQ files and return statistics, slightly different for Big K Small K
```{r, eval=FALSE}
temp = unlist(list.files(path=paste("data/", PROJECT, "/output/hsqs", sep=""), pattern="*.hsq"))
hsqs = t(sapply(X=temp, function(file_name) {
                  hsq = read.table(paste('data/', PROJECT, '/output/hsqs/', file_name, sep=''), header=TRUE, nrows=7, fill=TRUE)
